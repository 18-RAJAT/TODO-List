            ...
            <div className="task-container column">
                ...
                {toDoList?.length ?
                    toDoList.map((toDoObject, index) =>
                        ...
                        <ListItem key={index} itemData={toDoObject} markComplete={markComplete} index={index} deleteTask={deleteTask}/>) :
                    <p className="no-item-text"><span role="img" aria-label="react"></span> &nbsp;No Task Added📌</p>}
            </div>
            <p className="footer-text">®Rajat Joshi</p>
        </div>
    );
}
    ...
    return (
        <div className="list-item row jc-space-between">
            <span className={props.itemData.isComplete ? 'task-complete' : ''}
                 ...
                 <img className="delete-icon" src={trashIcon} alt="delete-task"
